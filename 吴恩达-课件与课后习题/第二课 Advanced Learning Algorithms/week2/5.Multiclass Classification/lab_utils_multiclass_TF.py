...
def plt_mc(X_train, y_train, classes, centers, std):
    ...
    plt_mc_data(ax, X_train, y_train, classes,
                map=dkcolors_map, legend=True, size=50, equal_xy=False)
...
def plt_layer_relu(X, Y, W1, b1, classes):
    ...
    fig, ax = plt.subplots(1, nunits, figsize=(7,2.5))
    for attr in ("toolbar_visible", "header_visible", "footer_visible"):
        if hasattr(fig.canvas, attr):
            setattr(fig.canvas, attr, False)
    ...
    for i in range(nunits):
        layerf = lambda x, i=i: np.maximum(0, np.dot(x, W1[:,i]) + b1[i])
        plt_prob_z(ax[i], layerf)
        plt_mc_data(ax[i], X, Y, classes,
                    map=dkcolors_map, legend=True, size=50, m='o')
